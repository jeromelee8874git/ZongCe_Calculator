...
test("允许自定义权重合计不等于 100%", () => {
  ...
  assert.equal(result.hasWeightAnomaly, true);
});

test("权重合计为 100% 时不标记异常", () => {
  const result = calculator.calculateTotal({}, { D: 10, X: 50, C: 20, S: 10, W: 10 });
  assert.equal(result.weightTotal, 100);
  assert.equal(result.hasWeightAnomaly, false);
});
