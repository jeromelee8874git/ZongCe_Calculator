...
test("修改任意权重后 P、标签和合计立即更新", () => {
  ...
  assert.equal(documentRef.elements["weight-warning"].hidden, false);
  assert.match(documentRef.elements["weight-warning"].textContent, /不是 100%/);
  assert.equal(documentRef.elements["weight-total"].classList.contains("is-warning"), true);
});

test("权重恢复为 100% 后清除异常提醒", () => {
  const documentRef = createFakeDocument();
  createScoreApp(documentRef, calculator);
  const weightX = documentRef.elements["weight-x"];
  ...
  weightX.value = "65";
  weightX.dispatch("input");
  assert.equal(documentRef.elements["weight-warning"].hidden, true);
  assert.equal(documentRef.elements["weight-warning"].textContent, "");
});
